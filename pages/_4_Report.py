...
with range_col:
    ...
    with st.form('range_form', clear_on_submit=False):
        ...
        range_manual_start_date_input, range_manual_end_date_input = st.columns(
            2)
        range_manual_start_date_input = range_manual_start_date_input.date_input(
            'From')
        range_manual_end_date_input = range_manual_end_date_input.date_input(
            'To')
        ...
        range_type = st.selectbox(label='Type', options=[
...
if range_type == 'Income and Expense':
    ...
    visualize_type = visualize_type.selectbox(
    ...
    with visualize_opt:
        with st.form('visualize_categories', clear_on_submit=False):
            ...
            visualize_cate_type = visualize_cate_type.selectbox(
                'Type', ['Income', 'Expense'])
            visualize_submit = st.form_submit_button('Submit')

visualization, ranking_income, ranking_expense = st.columns([2, 1, 1])
with visualization:
    if range_type == 'Income and Expense':
